...
public class Point {
	...
	public void inputPoints(List<Student> stus) {
		/*
		 1. 학생 객체를 1개 생성합니다.
...
//		stu.calcTotAvgGrade();	
	...
	public void showAllPoints(List<Student> stus) { //학생 객체들이 들어있는 리스트가 와야죠?
		...
		for(int i=0; i<stus.size(); i++) {
			total += stus.get(i).getAverage();
			stus.get(i).outputStuInfo();
		}
		
		System.out.println("현재 우리 반 평균: " + (double)total/stus.size());
	}
	...
	public void searchPoint(List<Student> students) {
		...
		System.out.println("그런 학번의 학생은 존재하지 않습니다.");
		sc.nextLine();
	}
